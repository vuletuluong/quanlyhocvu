          ...
          <div class="container-fluid">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb mb-0 py-3">
                ...
                <li class="breadcrumb-item active fw-light" aria-current="page">Thêm giảng viên</li>
              </ol>
            </nav>
          </div>
        </div>
              ...
              <div class="col-lg-12">
                <div class="card">
                  <div class="card-header">
                    ...
                    <h3 class="h4 mb-0">Thêm giảng viên</h3>
                  </div>
                  <div class="card-body">
                    <form action="" method="post" class="form-horizontal"  >
							...
							// Định nghĩa biến 
							$hotengiangvienerr = $id_taikhoanerr = $diachigverr = $sodienthoaierr = $emailgverr= "";
							$hotengiangvien= $id_taikhoan = $diachigv = $sodienthoai =$emailgv= "";
							...
							//Validate tên 
								...
								else 
								{
									...
									// Kiểm tra và chỉ cho phép nhập chữ và khoảng trắng 
									if (!preg_match("/^([a-zA-Z0-9ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s]+){4,16}$/",$id_taikhoan))
									{
										$id_taikhoanerr = "Họ tên giảng viên không hợp lệ";
									}
									else if($id_taikhoan!=$hotengiangvien)
									{
										
										$id_taikhoanerr = "Họ tên giảng viên không trùng nhau";
									}
						...
						?> 
                      <div class="row">
                        <label class="col-sm-3 form-label">Họ tên giảng viên</label>
                        <div class="col-sm-9">
                          <input name="hotengiangvien" class="form-control"   type="text">
                          <span class="error" style="color:red;">* <?php  echo $hotengiangvienerr; ?> </span>
                        </div>
                      </div>
                      ...
                      <div class="row">
                        ...
                        <div class="col-sm-9">
                          <input name="diachigv" type="text" class="form-control" id="diachigv">
                          <span class="error" style="color:red;">* <?php  echo $diachigverr; ?> </span>
                        </div>
                      </div>
                      <div class="border-bottom my-5 border-gray-200"></div>
                      <div class="row">
                        ...
                        <div class="col-sm-9">
                          ...
                          <span class="error" style="color:red;">* <?php  echo $sodienthoaierr; ?> </span>
                        </div>
                      </div>
                      <div class="border-bottom my-5 border-gray-200"></div>
                      <div class="row">
                        ...
                        <div class="col-sm-9">
                      	  <input name="emailgv" type="email" class="form-control" id="emailgv">
                          <span class="error" style="color:red;">* <?php  echo $emailgverr; ?> </span>
                        </div>
                      </div>
                       ...
                       <div class="row">
                        <label class="col-sm-3 form-label">Nhập lại họ tên giảng viên </label>
                        <div class="col-sm-9">
                          <input name="id_taikhoan" class="form-control"   type="text">
                          ...
                          <input name="nut" type="submit"  class="btn btn-primary" id="nut" value="Save changes">
							...
							switch($_POST['nut'])
							{
								case'Save changes':
								{
									...
									$hovaten=$_REQUEST['hotengiangvien'];
									$diachi=$_REQUEST['diachigv'];
									$email=$_REQUEST['emailgv'];
									$sdt=$_REQUEST['sodienthoai'];
									
									
									
									
								if($hotengiangvienerr == '' && $diachigverr == '' && $sodienthoaierr=='' && $emailgverr == '' && $id_taikhoanerr == '')
									{
					
											if($p->themxoasua("INSERT INTO giangvien (hoten, diachi, sdt,email, id_taikhoan) VALUES ( '$hovaten', '$diachi','$sdt', '$email',  '$id_taikhoan1');")==1)
											{
												echo'Thêm giảng viên thành công.';
											}
											else
											{
												echo'Thêm giảng viên không thành công.';
											}
									}
									else
									
									{
										/*echo ' Lỗi tên'. $hotengiangvienerr .'Lỗi địa chỉ'. $diachigverr .'Lỗi số điieenj thoai'. $sodienthoaierr .'lỗiemail'.$emailgverr .'lỗi id'.$id_taikhoanerr ;*/
										echo 'Thêm giảng viên không thành công.';
									}
									break;
								}

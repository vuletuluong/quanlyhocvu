                    ...
                    <div class="container-fluid">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0 py-3">
                                ...
                                <li class="breadcrumb-item active fw-light" aria-current="page">Xóa sửa tài khoản</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                            ...
                            <div class="col-lg-12">
                                <div class="card mb-0">
                                    <div class="card-header">
                                        ...
                                        <h3 class="h4 mb-0">Thông tin tài khoản</h3>
                                    </div>
                                    <div class="card-body">
                                        <div class="table-responsive">
                                            ...
                                            <?php 
												...
												$a->load_ds_taikhoan("select * from taikhoan where phanquyen ='giảng viên' OR phanquyen ='sinh viên' ");
												...
												if ($_SERVER["REQUEST_METHOD"] == "POST")
												 {
													...
													else 
													{
															...
															// Kiểm tra xem mật khẩu đã đúng định dạng hay chưa 
															if (!preg_match("/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/",$matkhau) ) {
																$matkhauerr = "Mật khẩu không hợp lệ mật khẩu tối thiểu tám ký tự, ít nhất một chữ cái, một số và một ký tự đặc biệt";
													}
										...
										<form action="" method="POST" class="form-control">
											...
											<div class="card-body">
												<div class="table-responsive">
													<table class="table mb-0">
														...
														<tbody>
															<tr>
																<input name="id_taikhoan" type="text" readonly id="id_taikhoan" value="<?php echo $a->laycot("SELECT id_taikhoan from taikhoan WHERE id_taikhoan='$layid' limit 1"); ?>" hidden="" />
																<td><input name="ten_taikhoan" type="text" readonly id="ten_taikhoan" value="<?php echo $a->laycot("SELECT username from taikhoan WHERE id_taikhoan='$layid' limit 1"); ?>" /></td>
																<td><input name="matkhau" type="text" id="matkhau" value="" /> <span class="error" style="color:red;">* <?php  echo $matkhauerr; ?> </span></td>
                                                                
																<td><input name="phanquyen" type="text" readonly id="phanquyen" value="<?php echo $a->laycot("SELECT phanquyen from taikhoan WHERE id_taikhoan='$layid' limit 1"); ?>" /></td>
                                                                <td><input name="ten_nguoidung" type="text" readonly id="ten_nguoidung" value="<?php echo $a->laycot("SELECT tennguoidung from taikhoan WHERE id_taikhoan='$layid' limit 1"); ?>" /></td>
															</tr>
											...
											<div class="row">
												<div class="col-sm-9 ms-auto">
                                                    ...
                                                    <input class="btn btn-success" onclick="myFunction() style="padding-left:15px;" type="submit" name="nut" id="nut" value="Sửa" />
											...
											switch ($_POST['nut']) {
												case 'Xóa'; {
														$idxoa = $_REQUEST['id_taikhoan'];
														if ($idxoa > 0) {
															if ($p->themxoasua("delete from taikhoan where id_taikhoan=$idxoa limit 1") == 1) {
																echo ' Thông báo xóa tài khoản thành công. ';
																echo' <script language="javascript"> window.alert("Xóa thành công"); </script>';
																echo '<script language="javascript">
																		window.location="xoataikhoan.php";
																		</script>';
															} else {
																echo 'Thất bại.';
															}
													...
													case'Sửa';
												{
													...
													$idsua=$_REQUEST ['id_taikhoan'];
													if($idsua >0)
													{
															if($matkhauerr=='')
															{
																if($p->themxoasua("UPDATE taikhoan SET password = '$matkhau'  WHERE id_taikhoan = $idsua limit 1;")==1)
																{
